 ...
 * @see WP_List_Table
 */
class Main_Admin_Ren extends WP_List_Table {
	...
	public function no_items() {
		...
		if ( 'moderated' === $comment_status ) {
			_e( 'No emails awaiting moderation.' );
		} elseif ( 'trash' === $comment_status ) {
			_e( 'No emails found in Trash.' );
		} else {
			_e( 'No emails found.' );
		}
	}
	...
	protected function get_views() {
		...
		$screen->base = 'admin.php?page=message';
		$status_links = array();
		...
		foreach ( $stati as $status => $label ) {
			...
			if('trash' === $status){
				$num_comments->trash = get_comments([
					'type'=> $this->com_type,
					'status' => 'trash',
					'count' => true,
				]);
	...
	/*protected function single_row_columns( $item ) {
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			if ( 'cb' === $column_name ) {
				...
				$screen = get_current_screen();
				$link = admin_url($screen->base);
				$link = add_query_arg('comment_id',$item->comment_ID,$link);
				$con ='';
				$con .= "<td $attributes><a href='$link'> ";
				$con .= call_user_func( array( $this, 'column_' . $column_name ), $item );
				$con .=  $this->handle_row_actions( $item, $column_name, $primary );
				$con .= '</a></td>';
				echo $con;
			} else {
	 ...
	 * @since 3.1.0
	 */
	public function do_extra_tablenav($do){
		$this->extra_tablenav = $do;
	}
	...
	public function single_row( $item ) {
		...
		$comment = $item;
		//$link = admin_url( $screen->base );
		//$link = add_query_arg('comment_id',$comment->comment_ID,$link);
	...
	protected function handle_row_actions( $comment, $column_name, $primary ) {
		...
		$actions = ['approve' => '', 'unapprove' => '', 'reply' => '', 'quickedit' => '', 'edit' => '', 'spam' => '', 'unspam' => '', 'trash' => '', 'untrash' => '', 'delete' => '',];
		...
		if('spam' !== $the_comment_status) {
			$actions['spam'] = sprintf('<a href="%s" data-wp-lists="%s" class="vim-s vim-destructive aria-button-if-js" aria-label="%s">%s</a>', $spam_url, "delete:the-comment-list:comment-{$comment->comment_ID}::spam=1", esc_attr__('Mark this comment as spam'), /* translators: "Mark as spam" link. */
				_x('Spam', 'verb'));
		}
		...
		foreach($actions as $action => $link) {
			...
			$sepLink = $sep.$link;
			$out .= "<span class='".$action."'>$sepLink</span>";
		}
...
/**
	...
	public function get_comm_detail($comm_id){
		$this->items = get_comment($comm_id);

	}

	public function view_email_form() {


		if($this->items->comment_approved == 0){

			wp_set_comment_status($this->items->comment_ID, 1);
		}
			$com_child = get_comments([
					'parent'=>$this->items->comment_ID,
			]);

		$this->user_can = current_user_can('edit_comment', $this->items->comment_ID);
		wp_nonce_field('fetch-list-'.get_class($this), '_ajax_fetch_list_nonce');
		static $has_items;
	...
	<table class="wp-list-table <?php echo implode(' ', $this->get_table_classes()); ?>">
		...
		<?php
			...
			$this->user_can = current_user_can('edit_comment', $this->items->comment_ID);
			?>
			<tr id="comment-<?php echo $this->items->comment_ID; ?>">
				<td><?php echo $this->items->comment_author; ?></td>
				<td><?php echo $sub = get_comment_meta($this->items->comment_ID,'subject',true) ? : 'Без темы'; ?></td>
				<td><?php echo $this->items->comment_date;  ?></td>
			</tr>
			<tr>
				<td colspan="3" align="center"><?php echo $this->items->comment_content; ?></td>
			</tr>
		<?php if(isset($com_child)): foreach($com_child as $com): ?>
		<tr>
			<td colspan="3">
				<p>Ответ <?php echo $this->items->comment_author?></p>
		...
		<?php endforeach; endif; ?>
			<tr>
				<td colspan="3">
					<?php echo $this->mes_handle_actions($this->items, $column_name = 'comment', $primary = 'comment'); ?>
				</td>
			</tr>
			</tbody>
			</table>
	...
	public function mes_handle_actions($comment, $column_name, $primary){
			global $comment_status;
		...
		if('spam' !== $the_comment_status && 'trash' !== $the_comment_status) {
			...
			$actions['reply'] = sprintf($format, $comment->comment_ID, $comment->comment_post_ID, 'replyto', 'vim-r message-reply', esc_attr__('Reply to this comment'), __('Reply'));
		}

			/** This filter is documented in wp-admin/includes/dashboard.php */
			//$actions = apply_filters('comment_row_actions', array_filter($actions), $comment);

			$out .= '<div class="col-md-4 mr-1 view-email-form-header row-actions visible">';
		...
		$actions = apply_filters('comment_row_actions', array_filter($actions), $comment);
			foreach($actions as $action => $link) {
				...
				$sepLink = $link;
				$out .= "<span class='px-3 ".$action."'>$sepLink</span>";
			}

			$out .= '</div>';

			do_action('mes_handle_actions');
